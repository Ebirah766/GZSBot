...
async def cmd_housed(ctx: commands.Context, *, zoo_name: str):
    ...
    async def cmd_unhoused(ctx, *, zoo_name: Optional[str] = None):
        """
        ;unhoused <zoo>
            List species that are HELD by <zoo> (from species_data['holdings']) but NOT currently housed by the user at that zoo.
        ;unhoused
        ;unhoused all
            Do the same for ALL zoos in the directory, one report with per-zoo sections.
        """
        try:
            data = _load_zoo_data()
            ...
            # ----- dataset access -----
            try:
                DS = SPECIES  # if you've set SPECIES = species_data
            except NameError:
                DS = species_data
            ...
            # ----- extract an institution name from a “holder piece” -----
            ...
            def alias_set_for_dir_key(dir_key: str) -> set[str]:
                rec = directory.get(dir_key, {})
                out = {norm_spaces(dir_key)}
                if isinstance(rec, dict):
                    als = rec.get("aliases")
                    if isinstance(als, list):
                        out |= {norm_spaces(a) for a in als if isinstance(a, str) and a.strip()}
                return out

            def resolve_display_label_and_aliases(requested: str) -> tuple[str, set[str]]:
                req_norm = norm_spaces(requested)
                # Prefer directory capitalization
                for dk in directory.keys():
                    if norm_spaces(dk) == req_norm:
                        return dk, alias_set_for_dir_key(dk)
                # Fallback to user’s own key if present
                user_zoos = (data.get("users", {}).get(user_id, {}) or {}).get("zoos", {})
                if isinstance(user_zoos, dict):
                    for uk in user_zoos.keys():
                        if norm_spaces(uk) == req_norm:
                            return uk, {norm_spaces(uk)}
                # Last resort: raw
                return requested.strip(), {req_norm}

            # ----- build a robust facility → set(species) index from DS['holdings'] -----
            # Only counts a holder if it has at least one letter (so "0", "0.0" etc. are ignored).
            FACILITY_INDEX: dict[str, set[str]] = {}

            def add_to_index(inst_name: str, species_common: str):
                k = norm_spaces(inst_name)
                if not re.search(r"[a-z]", k):  # require letters to avoid numeric noise
                    return
                FACILITY_INDEX.setdefault(k, set()).add(species_common)
            ...
            for common_name, entry in (DS or {}).items():
                ...
                # accumulate all holder pieces across regions
                for _region, raw in holdings.items():
                    ...
                    for piece in parts:
                        ...
                        if inst and re.search(r"[A-Za-z]", inst):
                            add_to_index(inst, str(common_name))

            # ----- species canonicalization (match by common OR scientific) -----
            # Build maps to match housed names typed by the user to dataset species.
            COMMON_TO_SCI: dict[str, str] = {}
            SCI_TO_COMMON: dict[str, str] = {}
            for common_name, entry in (DS or {}).items():
                sci = entry.get("scientific")
                if isinstance(sci, str) and sci.strip():
                    COMMON_TO_SCI[key_name(common_name)] = sci
                    SCI_TO_COMMON[key_name(sci)] = common_name

            def to_common_keyset(names: set[str]) -> set[str]:
                """
                Convert a set of user-stored species names (common or scientific)
                to the canonical common-name keys for subtraction.
                """
                out: set[str] = set()
                for n in names:
                    k = key_name(n)
                    if k in SCI_TO_COMMON:
                        out.add(key_name(SCI_TO_COMMON[k]))
                    else:
                        out.add(k)
                return out

            # ----- user housed lookup (union across similar keys) -----
            def housed_union_for_zoo(requested_label: str, alias_norms: set[str]) -> set[str]:
                user_zoos = (data.get("users", {}).get(user_id, {}) or {}).get("zoos", {})
                if not isinstance(user_zoos, dict):
                    return set()
                aliases_spaces = {norm_spaces(a) for a in (alias_norms | {requested_label})}
                aliases_keys = {key_name(a) for a in aliases_spaces}

                housed: set[str] = set()
                for k, val in user_zoos.items():
                    if not isinstance(val, list):
                        continue
                    ks = norm_spaces(k)
                    kk = key_name(k)
                    if (ks in aliases_spaces) or (kk in aliases_keys):
                        housed |= {str(x).strip() for x in val if str(x).strip()}
                return housed

            # ----- compute per-zoo result using the index -----
            def compute_unhoused_for_one(zreq: str) -> tuple[str, list[str]]:
                display_label, alias_norms = resolve_display_label_and_aliases(zreq)

                # collection = union of index buckets for any alias spelling
                alias_variants = {norm_spaces(display_label)} | set(alias_norms)
                collection_names: set[str] = set()
                for a in alias_variants:
                    collection_names |= FACILITY_INDEX.get(a, set())

                if not collection_names:
                    return display_label, []  # nothing held → nothing to report

                # subtract housed (match by common OR scientific names)
                housed_names = housed_union_for_zoo(display_label, alias_norms)
                housed_common_keys = to_common_keyset(housed_names)
                # collection map to common-name keys
                collection_map = {key_name(n): n for n in collection_names}
                # also map sci->common
                for k, v in list(collection_map.items()):
                    # if v is actually a scientific name present in map, translate
                    if key_name(v) in SCI_TO_COMMON:
                        cn = SCI_TO_COMMON[key_name(v)]
                        collection_map[key_name(cn)] = cn

                unhoused_keys = [k for k in collection_map.keys() if k not in housed_common_keys]
                unhoused = [collection_map[k] for k in sorted(unhoused_keys)]
                return display_label, unhoused

            # ----- run (single zoo or all) -----
            run_all = (zoo_name is None) or (str(zoo_name).strip().lower() in ("all","*"))

            if not run_all:
                zlabel, unhoused = compute_unhoused_for_one(str(zoo_name).strip())
                if not unhoused:
                ...
                buf.seek(0)
                await ctx.send(file=_discord.File(buf, filename=f"{key_name(zlabel)}_unhoused.txt"))
                return
            ...
            for zkey in sorted(directory.keys(), key=lambda s: s.lower()):
                zlabel, unhoused = compute_unhoused_for_one(zkey)
                if not unhoused:
                    continue
                count_zoos += 1
            ...
            if count_zoos == 0:
                await ctx.send("🎉 Across all zoos, there are no *unhoused* species (everything held is housed) or no holdings detected.")
                return

            header = f"Unhoused Species by Zoo — in collection but not housed\nZoos with results: {count_zoos}\n\n"
            content = header + "\n".join(sections)
